...
contract PreOrderToken is SimpleERC20 
{
    ...
    constructor(uint256 _intitialSupply,uint256 _tokenPrice,uint256 _saleDurationInSeconds,uint256 _minPurchase,uint256 _maxPurchase,address _projectOwner)SimpleERC20(_intitialSupply)
    {
    ...
    function transfer(address _to, uint256 _value)public override returns(bool)//override重写transfer()发售期间锁定
    {
        if(!finalized && msg.sender != address(this) && initialTransferDone)
        {
            require(false, "Tokens are locked until sale is finalized");
        }
        return super.transfer(_to, _value);//原始transfer()函数
    }

    function transferFrom(address _from, address _to, uint256 _value)public override returns(bool)//发售期间锁定
    {
    ...
    {
        buyTokens();//用户没有调用函数直接发送ETH时，调用函数发送代币
    }
}
